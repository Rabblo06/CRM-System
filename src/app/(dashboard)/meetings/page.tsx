    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full z-50 bg-white shadow-2xl flex flex-col" style={{ width: 440, borderLeft: '1px solid #DFE3EB' }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#DFE3EB]">
          <h2 className="text-base font-bold text-[#2D3E50]">Log a meeting</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">
              Meeting title <span className="text-[#FF7A59]">*</span>
            </label>
            <input
              ref={titleRef}
              value={title}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">Start time</label>
              <input
                type="datetime-local"
                value={startTime}
            ...
            <div>
              <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">Duration</label>
              <select
                value={duration}
          ...
          {/* Outcome */}
          <div>
            <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">Meeting outcome</label>
            <select
              value={outcome}
          ...
          {/* Assigned to */}
          <div>
            <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">Activity assigned to</label>
            <input
          ...
          {/* Notes */}
          <div>
            <label className="block text-xs font-semibold text-[#425B76] uppercase tracking-wide mb-1.5">Notes</label>
            <textarea
              value={notes}
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-[#DFE3EB] flex items-center gap-2">
          <button
            onClick={handleSave}
          ...
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-[#516F90] hover:text-[#2D3E50] transition-colors">
            Cancel
          </button>
        </div>
      </div>
...
function SortHeader({
  ...
  return (
    <button
      type="button"
      onClick={() => onSort(col)}
      className="flex items-center gap-1 group font-semibold uppercase tracking-wide hover:text-[#2D3E50] transition-colors"
      style={{ color: active ? '#2D3E50' : '#516F90', fontSize: 11 }}
    >
      {label}
      <ChevronDown
...
export default function MeetingsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* ── Top nav bar ── */}
      <div className="flex items-center justify-between px-5 py-2.5 border-b border-[#DFE3EB]" style={{ backgroundColor: '#fff' }}>
        <div className="flex items-center gap-1">
          {/* "Meetings" tab chip */}
          <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold border border-[#DFE3EB] rounded-[3px] text-[#2D3E50] hover:bg-[#F6F9FC] transition-colors">
            Meetings <ChevronDown className="w-3.5 h-3.5 text-[#99ACC2]" />
          </button>

          {/* "All records N ⋮" view tab */}
          <div className="flex items-center gap-1 ml-1">
            <button className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-[#2D3E50] border-b-2 border-[#2D3E50] transition-colors">
              All records
              <span className="flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold text-white" style={{ backgroundColor: '#2D3E50' }}>
                {meetings.length}
              </span>
            </button>
          ...
          {/* Add view */}
          <button className="flex items-center gap-1 px-2.5 py-1.5 text-sm text-[#516F90] hover:text-[#2D3E50] hover:bg-[#F0F3F7] rounded-[3px] transition-colors">
      ...
      <div className="flex items-center gap-2 px-5 py-2.5 border-b border-[#DFE3EB] bg-white">
        ...
        <div className="relative flex-1 max-w-lg">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#99ACC2]" />
          <input
            value={search}
            onChange={e => { setSearch(e.target.value); setPage(1); }}
            placeholder="Search"
            className="w-full h-8 pl-9 pr-3 text-sm border border-[#DFE3EB] rounded-[3px] outline-none text-[#2D3E50] placeholder:text-[#B0C1D4]"
        ...
        {/* View controls */}
        <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-[#DFE3EB] rounded-[3px] text-[#425B76] hover:bg-[#F6F9FC] transition-colors">
          Table view <ChevronDown className="w-3 h-3" />
        </button>
        <button className="p-1.5 rounded border border-[#DFE3EB] text-[#99ACC2] hover:text-[#425B76] hover:bg-[#F6F9FC] transition-colors">
        ...
        <button className="px-3 py-1.5 text-xs font-semibold border border-[#DFE3EB] rounded-[3px] text-[#425B76] hover:bg-[#F6F9FC] opacity-40 cursor-not-allowed transition-colors">
          Save
        </button>
      </div>

      {/* ── Quick column filters ── */}
      <div className="flex items-center gap-0 px-5 py-2 border-b border-[#DFE3EB] bg-white">
        {/* Meeting name sort */}
        <button
          onClick={() => handleSort('title')}
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-[3px] hover:bg-[#F6F9FC] transition-colors"
          style={{ color: sortCol === 'title' ? '#FF7A59' : '#2D3E50' }}
        >
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-[3px] hover:bg-[#F6F9FC] transition-colors"
          style={{ color: sortCol === 'created_at' ? '#FF7A59' : '#2D3E50' }}
        >
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-[3px] hover:bg-[#F6F9FC] transition-colors"
          style={{ color: sortCol === 'start_time' ? '#FF7A59' : '#2D3E50' }}
        >
        ...
        <div ref={outcomeRef} className="relative ml-1">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-[3px] hover:bg-[#F6F9FC] transition-colors"
            style={{ color: filterOutcome ? '#FF7A59' : '#2D3E50' }}
          >
            ...
            <div className="absolute top-full left-0 mt-1 z-40 bg-white border border-[#DFE3EB] rounded-[3px] shadow-xl py-1 min-w-[160px]">
              {OUTCOME_OPTS.map(o => (
                <button
                  ...
                  className="w-full px-4 py-2 text-xs text-left hover:bg-[#F6F9FC] transition-colors"
                  style={{ color: filterOutcome === o.value ? '#FF7A59' : '#2D3E50', fontWeight: filterOutcome === o.value ? 600 : 400 }}
                >
                  {o.label}
                </button>
              ))}
        ...
        {/* Advanced filters */}
        <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#0091AE] hover:text-[#007A99] transition-colors">
        ...
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-white z-10">
            <tr className="border-b border-[#DFE3EB]">
              ...
              <th className="px-4 py-3 text-left">
                <span className="font-semibold uppercase tracking-wide" style={{ color: '#516F90', fontSize: 11 }}>
                  Meeting end time
                </span>
              </th>
              <th className="px-4 py-3 text-left">
            ...
            {paginated.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center py-20">
                  <div>
                    <div className="w-12 h-12 rounded-full bg-[#F6F9FC] flex items-center justify-center mx-auto mb-4">
                      <svg className="w-6 h-6 text-[#DFE3EB]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                    </div>
                    <p className="text-sm font-semibold text-[#2D3E50] mb-1">No meetings found</p>
                    <p className="text-xs text-[#7C98B6] mb-4">
              ...
              return (
                <tr
                  key={activity.id}
                  className="border-b border-[#F0F3F7] hover:bg-[#F8FAFC] transition-colors group"
                >
                  {/* Checkbox */}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {details.outcome && (
                        <span
                          ...
                          style={{
                            backgroundColor: `${OUTCOME_COLORS[details.outcome] || '#DFE3EB'}20`,
                            color: OUTCOME_COLORS[details.outcome] || '#516F90',
                          }}
                        >
                  ...
                  {/* Start time */}
                  <td className="px-4 py-3 whitespace-nowrap" style={{ color: '#2D3E50' }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0" style={{ backgroundColor: '#FF7A59' }}>
                        {currentUserName?.[0]?.toUpperCase() || 'U'}
                      </div>
                      <span className="text-xs text-[#2D3E50] truncate max-w-[140px]">{currentUserName || '--'}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-1 rounded hover:bg-red-50 text-[#99ACC2] hover:text-red-400 transition-colors" title="Delete">
      ...
      {/* ── Pagination ── */}
      <div className="flex items-center justify-between px-5 py-3 border-t border-[#DFE3EB] bg-white flex-shrink-0">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setPage(p => Math.max(1, p - 1))}
            disabled={page === 1}
            className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium border border-[#DFE3EB] rounded-[3px] disabled:opacity-40 hover:bg-[#F6F9FC] text-[#425B76] transition-colors"
          >
            ← Prev
          </button>
