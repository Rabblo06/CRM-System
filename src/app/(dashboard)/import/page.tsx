      ...
      <div className="max-w-4xl mx-auto">
        ...
        {step !== 'done' && (
          <>
              ...
              {STEPS.map((s, i) => (
                <div key={s.id} className="flex items-center">
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                    s.id === step
                      ? 'bg-[#EEF0FB] border border-[#4762D5]/40'
                      : i < currentStepIndex
                      ? 'text-green-400'
                      : 'text-[#999999]'
                  }`}>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                      i < currentStepIndex
                        ...
                        : s.id === step
                        ? 'bg-[#4762D5] text-white'
                        : 'bg-[#F1F1F1] text-[#666666]'
                    }`}>
                      {i < currentStepIndex ? <Check className="w-3.5 h-3.5" /> : i + 1}
                    </div>
                    <span className={`text-sm font-medium ${s.id === step ? 'text-[#333333]' : ''}`}>
                      {s.label}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <ChevronRight className="w-4 h-4 text-[#B3B3B3] mx-1" />
                  )}
                </div>
              ))}
            </div>
            ...
            {step === 'type' && (
              <div>
                <h2 className="text-base font-semibold text-[#333333] mb-2">What would you like to import?</h2>
                <p className="text-sm text-[#666666] mb-6">Choose the type of records you want to import.</p>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`p-5 rounded-xl border-2 text-left transition-all ${
                        importType === type.id
                          ? 'border-[#4762D5] bg-[#EEF0FB]'
                          : 'border-[#EBEBEB] bg-white hover:border-[#EBEBEB]'
                      }`}
                    >
                      <div className={`${type.bg} w-14 h-14 rounded-xl flex items-center justify-center mb-3 ${type.color}`}>
                        {type.icon}
                      </div>
                      <p className="font-semibold text-[#333333] mb-1">{type.label}</p>
                      <p className="text-xs text-[#666666]">{type.desc}</p>
                      {importType === type.id && (
                        <div className="mt-2 flex items-center gap-1 text-[#4762D5] text-xs">
            ...
            {step === 'upload' && (
              <div>
                <h2 className="text-base font-semibold text-[#333333] mb-2">Upload your file</h2>
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-colors mb-4 ${
                    isDragging
                      ...
                      : 'border-[#EBEBEB] hover:border-[#B3B3B3] bg-[#FAFAFA]'
                  }`}
                >
                  <input
                    ref={fileRef}
                  ...
                  {file ? (
                    ...
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-14 h-14 bg-[#F1F1F1] rounded-xl flex items-center justify-center">
                        <Upload className="w-7 h-7 text-[#666666]" />
                      </div>
                      <div>
                        <p className="font-medium text-[#333333] mb-1">Drag and drop your file here</p>
                        <p className="text-sm text-[#666666]">or <span className="text-[#4762D5]">browse to choose a file</span></p>
                      </div>
                      <p className="text-xs text-[#999999]">Supports .xlsx, .xls, .csv</p>
                    </div>
                  )}
                </div>
            ...
            {step === 'map' && (
              <div>
                <h2 className="text-base font-semibold text-[#333333] mb-2">Map your columns</h2>
                <p className="text-sm text-[#666666] mb-4">
                  Match the columns in your file to the corresponding CRM fields.
                </p>

                <div className="rounded-xl border border-[#EBEBEB] overflow-hidden mb-5">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-[#F1F1F1] border-b border-[#EBEBEB]">
                        <th className="text-left px-4 py-3 text-[#666666] font-medium text-xs w-1/3">
                          File column
                        </th>
                        <th className="text-left px-4 py-3 text-[#666666] font-medium text-xs w-1/3">
                          Sample data
                    ...
                    <tbody className="divide-y divide-[#EBEBEB]">
                      {fileHeaders.map((header) => {
                        ...
                        return (
                          <tr key={header} className="hover:bg-[#F1F1F1]">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                {isMapped
                                  ? <CheckCircle2 className="w-4 h-4 text-green-400 flex-shrink-0" />
                                  : <div className="w-4 h-4 rounded-full border border-[#EBEBEB] flex-shrink-0" />
                                }
                                <span className="text-[#333333] font-medium">{header}</span>
                              </div>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-xs text-[#999999] truncate block max-w-[200px]">
                                {sample || '(empty)'}
                              </span>
                            </td>
                            <td className="px-4 py-3">
            ...
            {step === 'review' && (
              <div>
                <h2 className="text-base font-semibold text-[#333333] mb-2">Review your import</h2>
                <p className="text-sm text-[#666666] mb-6">
                ...
                <div className="space-y-4 mb-6">
                  <div className="bg-white border border-[#EBEBEB] rounded-xl p-5">
                    <h3 className="text-sm font-semibold text-[#333333] mb-3">Import summary</h3>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="text-center p-3 bg-[#FAFAFA] rounded-lg">
                        <p className="text-2xl font-bold text-[#333333]">{fileRows.length}</p>
                        <p className="text-xs text-[#666666]">Total rows</p>
                      </div>
                      <div className="text-center p-3 bg-[#FAFAFA] rounded-lg">
                        <p className="text-2xl font-bold text-[#4762D5]">
                          {Object.values(columnMapping).filter((v) => v && v !== '__skip__').length}
                        </p>
                        <p className="text-xs text-[#666666]">Mapped columns</p>
                      </div>
                      <div className="text-center p-3 bg-[#FAFAFA] rounded-lg">
                        <p className="text-2xl font-bold text-[#666666]">
                          {Object.values(columnMapping).filter((v) => v === '__skip__').length}
                        </p>
                        <p className="text-xs text-[#666666]">Skipped columns</p>
                      </div>
                    </div>
                  </div>

                  <div className="bg-white border border-[#EBEBEB] rounded-xl p-5">
                    <h3 className="text-sm font-semibold text-[#333333] mb-3">Column mappings</h3>
                        ...
                        .map(([col, field]) => {
                          ...
                          return (
                            <div key={col} className="flex items-center justify-between text-sm">
                              <span className="text-[#666666]">{col}</span>
                              <ChevronRight className="w-4 h-4 text-[#B3B3B3]" />
                              <span className="text-[#4762D5]">{fieldInfo?.label || field}</span>
                            </div>
                          );
                        })}
                    </div>
              ...
              <div className="flex justify-center gap-6 mb-8">
                {importResults.success > 0 && (
                  <div className="text-center p-4 bg-white border border-green-500/20 rounded-xl min-w-[100px]">
                    ...
                    <p className="text-sm text-[#666666]">Imported</p>
                  </div>
                )}
