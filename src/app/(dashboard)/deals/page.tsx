...
function WizardProgressBar({ subStep }: { subStep: SubStep }) {
  ...
  return (
    <div className="flex items-center border-b border-[#e5e8ed] px-8 py-3" style={{ backgroundColor: '#fff' }}>
      <span className="text-sm text-[#516f90] flex-shrink-0 w-44">Set up your sales process</span>
            ...
            <div key={i} className="flex items-center">
              <div className="flex flex-col items-center" style={{ width: 100 }}>
                <div
                  ...
                  style={{
                    borderColor: done || current ? '#ff7a59' : '#cbd6e2',
                    backgroundColor: done || current ? '#ff7a59' : '#fff',
                    color: done || current ? '#fff' : '#99acc2',
                  }}
                >
                  {done
                ...
                <span
                  ...
                  style={{ color: current ? '#2d3e50' : done ? '#ff7a59' : '#99acc2', fontWeight: current ? 600 : 400 }}
                >
                  {label}
                </span>
              </div>
              {i < 2 && (
                <div
                  ...
                  style={{ width: 60, backgroundColor: circleNum < active ? '#ff7a59' : '#dfe3eb', flexShrink: 0 }}
                />
              )}
            </div>
          );
      ...
      <span className="text-sm text-[#516f90] flex-shrink-0 w-24 text-right">
        Step {stepNum} of 3
      </span>
    </div>
  );
...
function TableKanbanPreview({ pipelineName, stages }: { pipelineName: string; stages: Stage[] }) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-[#2d3e50] mb-4" style={{ fontFamily: 'inherit' }}>
        {pipelineName || 'Your pipeline'}
      </h2>
      <div className="border border-[#dfe3eb] rounded-sm overflow-hidden" style={{ minHeight: 260 }}>
        <div className="flex" style={{ borderBottom: '1px solid #dfe3eb' }}>
          {allCols.map((col, i) => (
            <div
              key={col.id}
              className="flex-1 px-2 py-2 text-xs font-semibold text-[#516f90] truncate"
              style={{ borderRight: i < allCols.length - 1 ? '1px solid #dfe3eb' : 'none', minWidth: 0 }}
            >
              {col.name}
            </div>
          ))}
...
function DealCardPreview({ dealName, dealAmount, dealStage, stages }: {
  ...
  return (
    <div className="bg-white border border-[#dfe3eb] rounded-sm shadow-sm" style={{ width: 300 }}>
      {/* Deal name bar */}
      <div className="px-4 pt-5 pb-3 border-b border-[#dfe3eb]">
        {hasName
          ? <h3 className="text-base font-bold text-[#2d3e50]">{dealName}</h3>
          : <div className="h-5 rounded" style={{ backgroundColor: '#dfe3eb', width: '70%' }} />
        }
        <div className="mt-2 space-y-1">
          <div className="flex items-center gap-1.5 text-xs text-[#516f90]">
            <span className="text-[#7c98b6]">Amount:</span>
            {hasAmount
              ? <span className="font-medium text-[#2d3e50]">${Number(dealAmount).toLocaleString()}</span>
              : <div className="h-3 rounded inline-block" style={{ backgroundColor: '#dfe3eb', width: 80 }} />
            }
          </div>
          <div className="flex items-center gap-1.5 text-xs text-[#516f90]">
            <span className="text-[#7c98b6]">Stage:</span>
            {stageName
              ? <span className="font-semibold text-[#2d3e50]">{stageName}</span>
              : <div className="h-3 rounded inline-block" style={{ backgroundColor: '#dfe3eb', width: 100 }} />
            }
          </div>
        </div>
      </div>
      ...
      {/* Action icons */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[#dfe3eb]">
      ...
      <div className="px-4 py-3">
        <button type="button" className="flex items-center gap-1.5 text-xs font-semibold text-[#2d3e50] mb-3 w-full">
        ...
        {[
          ...
          <div key={row.label} className="mb-2">
            <div className="text-xs text-[#516f90] mb-0.5">{row.label}</div>
            <div
              className="h-7 rounded-sm px-2 flex items-center text-xs"
              style={{ backgroundColor: '#f5f8fa', color: row.value ? '#2d3e50' : '#b0bec5' }}
            >
              {row.value || ''}
            </div>
          </div>
...
export default function DealsPage() {
  ...
  const AddDealsBtn = () => (
    <div className="relative" ref={addDropRef}>
      <button
        ...
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.backgroundColor = '#2d3e50'; }}
      >
        Add deals <ChevronDown size={14} />
      </button>
      {showAddDropdown && (
        <div className="absolute right-0 top-10 z-50 bg-white border border-[#dfe3eb] rounded-[3px] shadow-lg py-1 min-w-[150px]">
          <button
            type="button"
            className="w-full text-left px-4 py-2 text-sm text-[#2d3e50] hover:bg-[#f6f9fc]"
            onClick={() => {
              setShowAddDropdown(false);
  ...
  if (view === 'empty') {
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#dfe3eb] bg-white flex-shrink-0">
          <div>
            <h1 className="text-xl font-bold text-[#2d3e50]">Deals</h1>
            <p className="text-sm text-[#7c98b6] mt-0.5">0 deals</p>
          </div>
          <AddDealsBtn />
        </div>
        <div className="flex-1 flex items-center justify-center bg-[#f6f9fc]">
          <div className="text-center" style={{ maxWidth: 380 }}>
            ...
            <svg width="180" height="100" viewBox="0 0 180 100" fill="none" className="mx-auto mb-5">
              {[0, 1, 2, 3].map(i => (
                <g key={i}>
                  <rect x={i * 44 + 4} y={8} width={36} height={10} rx={2} fill="#dfe3eb" />
                  <rect x={i * 44 + 4} y={24} width={36} height={50} rx={2} fill={i === 0 ? '#fff3f0' : '#fff'} stroke="#dfe3eb" />
                  {i === 0 && <>
                    <rect x={i * 44 + 9} y={30} width={22} height={4} rx={1} fill="#ff7a59" opacity={0.6} />
                    <rect x={i * 44 + 9} y={38} width={18} height={3} rx={1} fill="#dfe3eb" />
                    <rect x={i * 44 + 9} y={45} width={20} height={3} rx={1} fill="#dfe3eb" />
                  </>}
                </g>
              ))}
            </svg>
            <h2 className="text-lg font-bold text-[#2d3e50] mb-2">
              Create a deal to start building<br />your winning sales process
            </h2>
            <ul className="text-sm text-[#7c98b6] space-y-1.5 mb-6 text-left inline-block">
              {['Track deals through pipeline stages', 'Set amounts and close dates', 'See all deals in a visual board'].map(t => (
                <li key={t} className="flex items-start gap-2">
                  <span className="text-[#ff7a59] mt-0.5">•</span>{t}
                </li>
              ))}
            </ul>
            <br />
    ...
    return (
      <div className="flex flex-col h-full bg-[#f6f9fc]">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="bg-white border border-[#dfe3eb] rounded-lg shadow-xl" style={{ width: 580 }}>
            <div className="px-8 pt-8 pb-2 flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-[#2d3e50] mb-1">Set up your pipeline and create your first deal</h2>
                <p className="text-sm text-[#7c98b6]">We can help you create the stages of your deals pipeline and add a new deal.</p>
              </div>
              <button type="button" onClick={() => setView('empty')} className="p-1 text-[#99acc2] hover:text-[#425b76] ml-4 flex-shrink-0">
                <X size={18} />
              </button>
            </div>
              ...
              {[
                {
                  ...
                  svg: (
                    <svg width="64" height="64" viewBox="0 0 80 80" fill="none" className="mb-3">
                      <rect width="80" height="80" rx="8" fill="#fff3f0" />
                      <rect x="15" y="20" width="50" height="40" rx="3" fill="white" stroke="#dfe3eb" strokeWidth="1.5" />
                      <rect x="20" y="28" width="12" height="24" rx="1" fill="#ff7a59" opacity="0.3" />
                      <rect x="35" y="33" width="12" height="19" rx="1" fill="#ff7a59" opacity="0.5" />
                      <rect x="50" y="25" width="12" height="27" rx="1" fill="#ff7a59" opacity="0.8" />
                      <circle cx="56" cy="22" r="8" fill="#ff7a59" />
                ...
                },
                {
                  ...
                  svg: (
                    <svg width="64" height="64" viewBox="0 0 80 80" fill="none" className="mb-3">
                      <rect width="80" height="80" rx="8" fill="#f0f3f7" />
                      <rect x="25" y="18" width="30" height="38" rx="2" fill="white" stroke="#dfe3eb" strokeWidth="1.5" />
                      <rect x="30" y="24" width="20" height="3" rx="1" fill="#cbd6e2" />
                      <rect x="30" y="30" width="16" height="3" rx="1" fill="#cbd6e2" />
                      <rect x="30" y="36" width="18" height="3" rx="1" fill="#cbd6e2" />
                      <circle cx="57" cy="55" r="10" fill="#425b76" />
                ...
                <button
                  ...
                  className="flex-1 border border-[#dfe3eb] rounded-lg p-5 text-left hover:border-[#ff7a59] hover:shadow-md transition-all group"
                >
                  {opt.svg}
                  <div className="text-sm font-bold text-[#2d3e50] group-hover:text-[#ff7a59] transition-colors">{opt.title}</div>
                  <div className="text-xs text-[#7c98b6] mt-1 leading-relaxed">{opt.desc}</div>
                </button>
              ))}
            </div>
          </div>
  ...
  if (view === 'wizard') {
    ...
    return (
      <div className="flex flex-col h-full bg-white">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="overflow-y-auto" style={{ width: '50%', padding: '48px 64px 32px' }}>
            ...
            {subStep === 'name_pipeline' && (
              <div>
                <h2 className="text-3xl font-bold text-[#2d3e50] mb-4 leading-tight">
                  Set up your sales process<br />in minutes
                </h2>
                <p className="text-sm text-[#516f90] mb-2 leading-relaxed">
                ...
                <p className="text-sm mb-6 leading-relaxed" style={{ color: '#00a38d' }}>
                  The first step is to give your pipeline a clear and recognizable name.
                </p>
                <label className="block text-sm font-bold text-[#2d3e50] mb-2">Edit the name of your pipeline</label>
                <input
                  value={pipelineName}
                  onChange={e => setPipelineName(e.target.value)}
                  className="h-10 px-3 text-sm border border-[#cbd6e2] rounded-[3px] outline-none text-[#2d3e50]"
                  style={{ width: 340 }}
            ...
            {subStep === 'define_stages' && (
              <div>
                <h2 className="text-3xl font-bold text-[#2d3e50] mb-4 leading-tight">Define your deal stages</h2>
                <p className="text-sm mb-2 leading-relaxed" style={{ color: '#516f90' }}>
                  Deal stages are the steps in your pipeline. They mark the real-life milestones in your sales process.
                </p>
                <p className="text-sm mb-6 leading-relaxed" style={{ color: '#516f90' }}>
                  Start with our suggestions, or rename them as you like. You can also add, remove or rename stages later.
                </p>
                <div className="font-bold text-sm text-[#2d3e50] mb-3">Stage name</div>
                  ...
                  {stages.map((stage) => (
                    <div key={stage.id} className="flex items-center gap-2 group">
                      <input
                        ...
                        className="flex-1 h-10 px-3 text-sm border border-[#cbd6e2] rounded-[3px] outline-none text-[#2d3e50]"
                        onFocus={e => { e.currentTarget.style.borderColor = '#3b82f6'; e.currentTarget.style.boxShadow = '0 0 0 1px #3b82f6'; }}
                        onBlur={e => { e.currentTarget.style.borderColor = '#cbd6e2'; e.currentTarget.style.boxShadow = 'none'; }}
                      />
                      {stages.length > 1 && (
                        <button
                          ...
                          className="p-1.5 text-[#99acc2] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
            ...
            {subStep === 'closing_stages' && (
              <div>
                ...
                <div className="flex gap-6 mb-2" style={{ maxWidth: 420 }}>
                  <div className="flex-1 text-sm font-bold text-[#2d3e50]">Closing stage names</div>
                  <div className="text-sm font-bold text-[#2d3e50] flex items-center gap-1">
                    Deal outcome
                    <span className="w-4 h-4 rounded-full border border-[#cbd6e2] text-[#99acc2] text-[10px] flex items-center justify-center cursor-help">i</span>
                  </div>
                </div>
                <div className="space-y-3" style={{ maxWidth: 420 }}>
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      onBlur={e => { e.currentTarget.style.borderColor = '#cbd6e2'; e.currentTarget.style.boxShadow = 'none'; }}
                    />
                    <div className="h-10 px-3 flex items-center text-sm text-[#99acc2] border border-[#dfe3eb] rounded-[3px] bg-[#f6f9fc]" style={{ minWidth: 80 }}>Won</div>
                  </div>
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      onBlur={e => { e.currentTarget.style.borderColor = '#cbd6e2'; e.currentTarget.style.boxShadow = 'none'; }}
                    />
                    <div className="h-10 px-3 flex items-center text-sm text-[#99acc2] border border-[#dfe3eb] rounded-[3px] bg-[#f6f9fc]" style={{ minWidth: 80 }}>Lost</div>
                  </div>
                </div>
                <div className="mt-6">
                  <button
            ...
            {subStep === 'add_deal' && (
              <div>
                <h2 className="text-3xl font-bold text-[#2d3e50] mb-4 leading-tight">Add a deal</h2>
                <p className="text-sm mb-2 leading-relaxed" style={{ color: '#516f90' }}>
                ...
                <div className="space-y-4" style={{ maxWidth: 340 }}>
                  <div>
                    <label className="block text-sm font-bold text-[#2d3e50] mb-2">Deal name</label>
                    <input
                      value={dealName}
                      onChange={e => setDealName(e.target.value)}
                      placeholder="e.g. Consulting: Acme Corp (Sample Deal)"
                      className="h-10 w-full px-3 text-sm border border-[#cbd6e2] rounded-[3px] outline-none text-[#2d3e50] placeholder:text-[#b0c1d4]"
                      onFocus={e => { e.currentTarget.style.borderColor = '#3b82f6'; e.currentTarget.style.boxShadow = '0 0 0 1px #3b82f6'; }}
                      onBlur={e => { e.currentTarget.style.borderColor = '#cbd6e2'; e.currentTarget.style.boxShadow = 'none'; }}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-[#2d3e50] mb-2">Amount</label>
                    <input
                      type="number"
                      value={dealAmount}
            ...
            {subStep === 'choose_stage' && (
              <div>
                <h2 className="text-3xl font-bold text-[#2d3e50] mb-4 leading-tight">Get into the details</h2>
                <p className="text-sm mb-6 leading-relaxed" style={{ color: '#516f90' }}>
                ...
                <div style={{ maxWidth: 340 }}>
                  <label className="block text-sm font-bold text-[#2d3e50] mb-2">Deal stage</label>
                  <select
                    value={dealStage}
                    onChange={e => setDealStage(e.target.value)}
                    className="h-10 w-full px-3 pr-8 text-sm border border-[#cbd6e2] rounded-[3px] outline-none text-[#2d3e50] bg-white appearance-none"
          ...
          {/* Right: live preview */}
          <div className="flex-1 overflow-y-auto bg-[#f5f8fa]" style={{ borderLeft: '1px solid #dfe3eb' }}>
        ...
        {/* Bottom bar */}
        <div className="flex items-center justify-between px-8 py-4 border-t border-[#dfe3eb] bg-white flex-shrink-0">
          {subStep !== 'name_pipeline' ? (
            <button
              type="button"
              onClick={handleBack}
              className="flex items-center gap-1 text-sm font-semibold text-[#425b76] hover:text-[#2d3e50] border border-[#dfe3eb] px-3 py-1.5 rounded-[3px]"
            >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#dfe3eb] bg-white flex-shrink-0">
        <div>
          <h1 className="text-xl font-bold text-[#2d3e50]">{pipelineName}</h1>
          <p className="text-sm text-[#7c98b6] mt-0.5">{deals.length} deal{deals.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={async () => { localStorage.removeItem(userKey('crm_pipeline_v2')); await deleteAllDeals(); resetWizard(); setView('empty'); }}
            className="text-xs text-[#99acc2] hover:text-[#425b76] transition-colors"
          >
            Reset pipeline
          </button>
          <AddDealsBtn />
